...
## Title
D009 add Playwright smoke tests for matched and no-match chat flows

## Scope
test

## Summary
Add Playwright `@smoke` coverage for the new chat-driven goal creation surface, covering both the matched `youtube_video` path through goal creation and the no-match path through the stubbed `501` request-new-goal-type response.
...
- [ ] T1: Identify the existing Playwright test harness, auth/bootstrap pattern, and any helper utilities required to open the authenticated app shell and reach the create-goal entrypoint. (AC: 1, 2)
  - [ ] T1.1: Confirm the frontend now routes home-screen "Create goal" to `frontend/screens/ChatGoalCreateScreen.tsx`, not `GoalCreateScreen.tsx`. (AC: 1, 2)
  - [ ] T1.2: Confirm the smoke tests target the shipped chat UI and not removed typed creation UI. (AC: 1, 2)
- [ ] T2: Add a Playwright `@smoke` test for the matched path. (AC: 1)
  - [ ] T2.1: Open the chat creation screen from the home screen. (AC: 1)
  - [ ] T2.2: Send the prompt `I want to upload a YouTube walkthrough of my project by Friday and pledge $20 to <charity>`. (AC: 1)
  - [ ] T2.3: Assert the assistant surfaces a match card for `youtube_video`. (AC: 1)
  - [ ] T2.4: Provide any missing criteria conversationally as prompted by the assistant. (AC: 1)
  - [ ] T2.5: Confirm final creation action. (AC: 1)
  - [ ] T2.6: Assert a new goal exists via `GET /api/goals` with `goal_type=youtube_video`. (AC: 1)
- [ ] T3: Add a Playwright `@smoke` test for the no-match path. (AC: 2)
  - [ ] T3.1: Open the chat creation screen. (AC: 2)
  - [ ] T3.2: Send the prompt `Track that I drank 8 glasses of water today`. (AC: 2)
  - [ ] T3.3: Assert the assistant returns the build-a-new-goal-type affordance. (AC: 2)
  - [ ] T3.4: Tap the no-match affirmative action. (AC: 2)
  - [ ] T3.5: Assert the stubbed `501` response is surfaced as an honest assistant message in chat and the UI does not crash. (AC: 2)
- [ ] T4: Keep smoke coverage narrow and deterministic. (AC: 1, 2)
  - [ ] T4.1: Reuse stable selectors/assertions for structured assistant action cards rather than incidental text layout where possible. (AC: 1, 2)
  - [ ] T4.2: Avoid coupling to proof submission flows, since proof submission is out of scope and unchanged in this direction. (AC: 1, 2)
  - [ ] T4.3: Ensure tests are tagged `@smoke`. (AC: 1, 2)
...
- This story is test-only. Do not implement backend or frontend feature work here; validate already-landed contracts and shipped UI behavior.
- Proof submission screens remain unchanged in this direction and should not be included in smoke coverage except as untouched surrounding product context.
- The test should assert behavior through the user-facing chat surface plus `GET /api/goals` verification for the matched path, per direction.
- If stable selectors for structured action cards do not exist yet, use the minimum reliable selectors available in the shipped UI; avoid brittle layout-only assertions.
