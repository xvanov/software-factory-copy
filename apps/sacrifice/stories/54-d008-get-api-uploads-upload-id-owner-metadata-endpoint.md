...
## Goal
Add owner-scoped upload metadata read endpoint `GET /api/uploads/{upload_id}` that returns the exact contract for the owning user and rejects non-owners or unknown ids with the specified statuses.
...
- [ ] Add `GET /api/uploads/{upload_id}` route.
- [ ] Enforce authentication.
- [ ] Load upload record by `upload_id`.
- [ ] Return `404` for unknown ids.
- [ ] Return `403` when upload exists but is not owned by authenticated user.
- [ ] Return `200` with response body matching `api_spec.md` exactly for owner.
- [ ] Keep route behavior aligned with existing backend auth/serialization conventions.
...
- This story assumes upload records already exist via earlier stories.
- Match ownership semantics exactly: 403 for existing non-owned record, 404 for unknown id.
- Response shape must use `upload_id` rather than raw internal field names.
