...
## Goal
Implement `backend/app/services/uploads.py` as the single backend abstraction for media upload path resolution, file write, SHA-256 hashing, and metadata persistence so later routes remain thin and testable.
...
- [ ] Implement path resolution using configured media root and `(user_id, goal_id_or_unassigned, upload_id)` layout.
- [ ] Implement file write behavior for whole-file upload persistence.
- [ ] Implement SHA-256 computation for stored content.
- [ ] Persist upload metadata through the `media_uploads` model introduced by the prior story.
- [ ] Structure service API so route handlers can delegate business logic without embedding write/hash/path logic.
- [ ] Add unit tests for service behavior covering pathing, hashing, write, and persistence seams.
- [ ] Do not add HTTP route behavior in this story beyond any minimal imports needed for testability.
...
- Depends on: D008 media_uploads model + config + Alembic migration
- Unblocks:
...
- [ ] Service owns path resolution, write, hash, and persistence.
- [ ] Path layout matches direction exactly.
- [ ] Service is unit-testable without route coupling.
- [ ] Route logic is not implemented here.
