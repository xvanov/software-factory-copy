...
## Tasks / Subtasks
- [ ] Add `awaiting_goal_type` to backend goal status enum definitions
- [ ] Add nullable `awaiting_direction_id` column to `goals`
- [ ] Generate Alembic migration via autogenerate
- [ ] Apply model/schema updates needed for persisted linkage
- [ ] Add tests for enum persistence and nullable linkage behavior
- [ ] Verify existing goal lifecycle paths remain intact
