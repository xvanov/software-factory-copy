# Story
## Title
D010 generation-status endpoint maps state.yaml to coarse states
...
- [ ] Resolve in-flight direction from chat session / pending goal linkage
- [ ] Read direction `state.yaml` from configured directions volume
- [ ] Map factory lifecycle states to API coarse statuses explicitly
- [ ] Return `pr_url` when available
- [ ] Return stable summary field if specified by route contract
- [ ] Add tests for each coarse status mapping and 401/404 paths
...
## References
- chat/session route file from D009
- direction state reader helper/service
- configured directions path consumer
