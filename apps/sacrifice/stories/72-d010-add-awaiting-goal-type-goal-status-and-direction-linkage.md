...
## Story
As the backend,
I want a persisted pending-goal lifecycle state and direction linkage,
so that generated goal-type requests have canonical storage before activation.
...
## Tasks / Subtasks
- Add `awaiting_goal_type` to `Goal.status` enum.
- Add nullable `awaiting_direction_id` to `goals` model.
- Generate Alembic migration via autogenerate.
- Update schema serialization for new goal status if required.
- Add backend tests for enum persistence and nullable linkage.
- Verify existing active-goal behavior remains unchanged.
- Document any temporary assumptions in Dev Agent Record.
