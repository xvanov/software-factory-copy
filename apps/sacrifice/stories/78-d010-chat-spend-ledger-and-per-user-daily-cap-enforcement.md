...
I want LLM-backed chat generation and iteration calls recorded against a daily spend ledger with enforced caps,
so that Sacrifice limits cost exposure before expensive synthesis work begins.
...
## Tasks / Subtasks
- [ ] Add persistence for `chat_spend_ledger` with fields sufficient for per-user, per-call millicent accounting.
- [ ] Add migration(s) for the new spend ledger table.
- [ ] Add configurable per-user daily cap setting with default `$1.00` equivalent in millicents.
- [ ] Implement preflight cap check before LLM-backed generation/iteration synthesis calls.
- [ ] Record each LLM call cost to the ledger on successful or billable invocation paths.
- [ ] Return `429 Too Many Requests` with a clear message once the per-user daily cap is exhausted.
- [ ] Add tests for under-cap success, exact-cap boundary, over-cap rejection, and per-user/per-day isolation.
