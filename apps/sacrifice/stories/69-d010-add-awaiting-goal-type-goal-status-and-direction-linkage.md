...
## Goal
Add `awaiting_goal_type` to `Goal.status` and persist nullable direction linkage so pending generated goals have canonical storage before endpoint and worker integration.
...
## Tasks / Subtasks
- [ ] Add `awaiting_goal_type` to goal status enum in model and schema surfaces touched by persistence.
- [ ] Add nullable `awaiting_direction_id` column on `goals`.
- [ ] Generate Alembic migration via autogenerate.
- [ ] Update ORM create/read paths to preserve `awaiting_direction_id`.
- [ ] Add backend tests for enum persistence and nullable linkage behavior.
- [ ] Verify existing goal statuses remain unchanged.
- [ ] Confirm pending generated goals can exist without active verification type activation.
