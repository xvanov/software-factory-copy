...
## Title
D009 frontend chat goal creation screen and routing

## Scope
frontend

## Summary
Replace the home-screen goal creation entry with `frontend/screens/ChatGoalCreateScreen.tsx`, remove `frontend/screens/GoalCreateScreen.tsx`, render structured assistant action cards from the chat API, and resume an existing chat session locally without changing proof submission screens.
...
- [ ] Replace create-goal routing entrypoints with chat-first navigation.
  - [ ] Update the home-screen create affordance to navigate to `frontend/screens/ChatGoalCreateScreen.tsx`.
  - [ ] Update navigation route definitions and hook usage so no route points at `frontend/screens/GoalCreateScreen.tsx`.
  - [ ] Preserve proof-submission navigation unchanged.
- [ ] Implement `frontend/screens/ChatGoalCreateScreen.tsx`.
  - [ ] Create screen shell with message list, composer input, send action, and typing/loading state.
  - [ ] Disable send for empty or whitespace-only input.
  - [ ] On first entry, create or resume a chat session and render the greeting returned by `POST /api/chat/sessions` or the persisted session transcript.
  - [ ] Persist the active `session_id` locally for resume-on-return behavior.
- [ ] Render structured assistant cards from backend `action` payloads.
  - [ ] Render `match_proposed` as a "Use this goal type" card with matched goal type context and actions for proceed/rephrase.
  - [ ] Render `no_match` as a "Build a new goal type" card with actions for stub call / rephrase.
  - [ ] Render `awaiting_input` as a single-field prompt in chat flow.
  - [ ] Render `ready_to_create` as a final review card with create/edit actions.
- [ ] Wire chat screen API interactions in frontend services.
  - [ ] Call `POST /api/chat/sessions` to start a session when needed.
  - [ ] Call `POST /api/chat/sessions/{session_id}/messages` for each user turn.
  - [ ] Call `POST /api/chat/sessions/{session_id}/create-goal` from final review confirmation.
  - [ ] Call `POST /api/chat/sessions/{session_id}/request-new-goal-type` for the no-match branch and surface the stubbed `501` detail honestly in chat.
  - [ ] Retry the last message when matching returns transient failure semantics.
- [ ] Remove typed creation screen artifacts.
  - [ ] Delete `frontend/screens/GoalCreateScreen.tsx`.
  - [ ] Remove imports, route references, and dead helper code tied only to typed creation.
- [ ] Verify frontend behavior against downstream test story expectations.
  - [ ] Ensure UI supports both matched and no-match paths without crashing.
  - [ ] Ensure successful create-goal flow navigates to goal detail after backend returns `goal_id`.
...
- Proof submission screens remain unchanged in this story; only creation entry and creation UI move to chat.
- Frontend should consume backend `action` payloads as typed variants rather than inferring state from freeform assistant text.
- If local session-resume storage shape is introduced or changed, keep it minimal and isolated to chat creation flow.
...
## Review Notes
- Verify all typed creation references are removed.
- Verify card rendering follows structured action payloads exactly.
- Verify resume flow does not interfere with auth or proof submission navigation.
